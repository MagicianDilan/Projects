...
ang(1)=30;           % ángulo de salida [unidades grados]
...
for i=2:n
    ...
    x(i,1) = x(i-1,1) + vc(i,1)*cos(ang(1))*h;
    y(i,1) = y(i-1,1) + vc(i,1)*sin(ang(1))*h;
...
vt=sqrt((2*mb*g)/(cd*p*A));   % Velocidad terminal del cohete [unidades m/s]

ay = zeros(i,1);            %Creamos los vectores de aceleración y velocidades
vx = zeros(i,1);
vy = zeros(i,1);

vx(i,1) = vc(i,1)*cos(ang(1));  %Inicializamos las variables para el ciclo en el que nos quedamos
vy(i,1) = vc(i,1)*sin(ang(1));
ay(i,1) = -g;
for i=1:i
    ang(i,1) = ang(1);
end
...
% % CÓDIGO QUE SOLO CARGA LA IMAGEN DEL COHETE
